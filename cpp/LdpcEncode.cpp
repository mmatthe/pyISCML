...
static void encode(int u[], int H_rows[], int *c_in,int nldpc,int kldpc,int mldpc, int wid_Hrows, int P[], int shift)
{
...
extern "C" void ldpcEncode(int* bits, int sizeBits,
			   int* H_rows, int sizeR2, int sizeR1,
			   ...
			   int* encoded, int sizeEnc)
{
  ...
  int kldpc = sizeBits;
  int mldpc = sizeR1;
  int wid_Hrows = sizeR2;

  int nldpc = kldpc + mldpc;

  encode(bits, H_rows, encoded, nldpc, kldpc, mldpc, wid_Hrows, P, shift);
}
