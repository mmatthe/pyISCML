...
#ifndef MATLAB
void mexErrMsgTxt(const char* txt)
{
						 ...
						 int      data[] )
{
	...
	for (iter=0;iter<max_iter;iter++) {
		...
		for (i=0;i<CodeLength-NumberParityBits;i++){
		  int b = llrOut[iter+max_iter*i] < 0;
			if ( b != data[i] )
				BitErrors[iter]++;
		}
			 ...
			 int* BitErrors, int sizeBE)
{
  ...
  int i, j, count, cnt, k, c_index, v_index;
  ...
  struct c_node* c_nodes = (struct c_node*)calloc( NumberParityBits, sizeof( struct c_node ) );
  ...
  struct v_node* v_nodes = (v_node*)calloc( CodeLength, sizeof( struct v_node));
  ...
  if ( dec_type == 1) {
    MinSum( BitErrors, llrOut, c_nodes, v_nodes, CodeLength,
    ...
    SumProduct( BitErrors, llrOut, c_nodes, v_nodes, CodeLength,
		NumberParityBits, max_iter, r_scale_factor, q_scale_factor, data_int );
  }
		 ...
		 const mxArray *prhs[] )
{
  ...
  if (nrhs > 7 ) {
    ...
    std::copy(data, data + DataLength, data_int);
  }



  /* Decode */
  int* DecodedBits = (int*)calloc( max_iter*CodeLength, sizeof( int ) );
  int* BitErrors = (int*)calloc( max_iter, sizeof(int) );
  ...
  MpDecode(input, CodeLength,
	   H_rows, NumberParityBits, max_row_weight,
	   H_cols, NumberRowsHcols, max_col_weight,
	   max_iter, dec_type, r_scale_factor, q_scale_factor,
	   data_int, DataLength,
	   DecodedBits, max_iter, CodeLength,
	   BitErrors, max_iter);
  ...
  if (nlhs > 1 ) {
    ...
    double* errors_p = mxGetPr(ERRORS);
    std::copy(BitErrors, BitErrors + max_iter, errors_p);
  }

  /* create matrices for the decoded data */
  OUTPUT = mxCreateDoubleMatrix(max_iter, CodeLength, mxREAL );
  double* output_p = mxGetPr(OUTPUT);
  std::copy(DecodedBits, DecodedBits+max_iter*CodeLength, output_p);
